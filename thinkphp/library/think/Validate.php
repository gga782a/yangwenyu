...
//        dd($this->message);
        ...
        foreach ($rules as $key => $rule) {
            ...
            if (false === $result) {
                if (isset($msg[$rule])) {
                    ...
                    $message    =   $fieldname.' '.$this->message[$rule];
    ...
    protected function exits($value, $rule, $data, $field)
    {
        if (is_string($rule)) {
            $rule = explode(',', $rule);
        }
    ...
    protected function getRuleMsg($attribute, $title, $type, $rule)
    {
        ...
        } elseif (!empty($this->message[$type]))
        {
            $msg = ':attribute'.$this->message[$type];
        ...
        if (is_string($msg) && is_scalar($rule) && false !== strpos($msg, ':')) {
            ...
            {
                case 'length':
                    $msg        =   str_replace(['{min}','{max}'],[$array[0],$array[1]],$msg);
                    break;
            }
            $msg = str_replace(
